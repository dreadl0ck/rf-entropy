...
This repo contains notes and source code for an experimental random number generator based on RTL SDR hardware.

The random numbers generated are most likely random enough for non-important applications. Please note that this is in no way verified to be Cryptographically Secure, so it should not be used for cryptographic purposes.

Known vulnerability: We noticed that we could influence the rate at which data is generated by sending RF signals to the SDR device, possibly to the point that no output is generated at all.
